...
module.exports = NodeHelper.create({
	start() {
		...
		this.messageHistory = [{ role: "system", content: "You are a helpful voice assistant for a smart mirror." }];
	},
	...
	recordAudio() {
		...
		const inputType = isWindows ? "waveaudio" : "alsa"; // or "pulseaudio" if needed
		const micDevice = isWindows ? "-d" : "default";

		const command = `sox -t ${inputType} ${micDevice} ${filePath} trim 0 5`;
	...
	async transcribeAudio(filePath) {
		...
		try {
			...
			// 🔍 Log the raw error from OpenAI
			console.error("[GPT-Voice] Whisper API error object:", error.response?.data || error);

			// ✅ Extract a useful message
			const message = error.response?.data?.error?.message || error.message || "Unknown Whisper error";
			console.log("[GPT-Voice] Whisper error message:", message);

			// 🔁 Return to UI
			this.sendSocketNotification("GPT_RESPONSE", {
				response: "Whisper failed: " + message
			});
		}
	},
	async queryChatGPT(prompt) {
		...
		if (this.messageHistory.length > 20) {
			this.messageHistory = [this.messageHistory[0], ...this.messageHistory.slice(-18)];
		}
